    ...
    <div id="client-list">
                  ...
                  <li key={item._id}>
                    <div className="info">
                      <span
                        className="name"
                        onClick={() =>
                          setClientInfo({ status: true, info: item })
                        }
                      >
                        {item.fullName}
                      </span>
                      <span style={{ fontWeight: "bold" }}>
                        {getMedal(item.totalMoney)}
                      </span>
                      <span>
                    ...
                    <div className="tools">
                      <button
                        onClick={() => {
                          setProduct({ user: item, status: true });
                        }}
                        className="add"
                      >
      ...
      {guestPage.totalPage > 1 && (
        <div className="client-pagination">
          <Pagination
            totalPage={guestPage.totalPage}
            currentPage={guestPage.currentPage}
